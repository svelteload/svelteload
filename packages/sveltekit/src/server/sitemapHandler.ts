...
const computeLastmod = (updatedAt: string, floor?: string): string => {
    const contentDate = new Date(updatedAt)
    if (floor) {
        const floorDate = new Date(floor)
        if (contentDate < floorDate) return floorDate.toISOString().split('T')[0]
    }
...
export const createSitemapHandler = (config: SitemapHandlerConfig): RequestHandler => {
    return async () => {
        ...
        if (localization) {
            ...
            for (const doc of docs) {
                ...
                const lastmod = computeLastmod(doc.updatedAt, config.lastmodFloor)
        ...
        for (const doc of docs) {
            for (const path of Object.values(doc.localizedPaths)) {
                ...
                urls.push({
                    ...
                    lastmod: computeLastmod(doc.updatedAt, config.lastmodFloor),
                })
            }
        }
